# CLAUDE.md — 교육과정 지식그래프 스키마 (Curriculum Second Brain)

이 폴더는 **2022 개정 교육과정을 LLM이 유지·관리하는 지식 위키**다. 개인 second brain의
3계층 구조(Karpathy식 LLM Wiki + 타입 지식그래프)를 **교육과정용으로 이식**하되, 핵심 차이는
하나다: **성취기준을 그대로 노드로 두지 않고, 가르치고 진단할 수 있는 가장 작은 "개념 원자"로
잘게 분해한다.** 목표는 다문화 학생의 **선수학습 결손을 개념 단위로 추적·보완**하는 자산이며,
옵시디언 그래프로 시연하고 ClassTalk 등 여러 앱에서 재사용한다.

> ⚠️ 이 자산은 **공용 GitHub 레포에 올리지 않는다.** 나중에 개인 계정 private repo로 이전.
> 원천 PDF는 `../2022개정교육과정(깃헙에 올리지마_자산구축용)/`에 있다(읽기 전용).
...
| 계층 | 폴더 | 소유자 | 규칙 |
|------|------|--------|------|
| **Raw sources** | `10-sources/` | 사용자 | **읽기 전용.** 원천 별책 PDF 목록·메타. 진실의 원천. 수정 금지. |
...
- `standards/*.json` = **기계용 coarse 내보내기(성취기준 수준)**. 앱(ClassTalk)이 현재 소비.
  **단일 출처는 `20-nodes/`(세밀 그래프)**이며, 성숙하면 노드→JSON 생성으로 대체한다.
...
| type | 의미 | 예시 | 비고 |
|------|------|------|------|
| `concept` | 개념·정의(명사형 원자) | 통분, 공통분모, 기약분수, 약수, 진분수 | 가장 많음. 결손 추적의 단위. |
| `skill` | 절차·기능(동사형 how-to) | 통분하기, 분모가 다른 분수 더하기, 약분하기 | 한 절차 = 한 노드. |
| `standard` | 성취기준(공식 단위) | `[6수01-04]` 분모가 다른 분수의 덧셈과 뺄셈 | **verbatim**. 개념·스킬을 묶는 허브. |
| `misconception` | 흔한 오개념·결손 패턴 | "분모끼리 더한다", "약분·통분 혼동" | ★ 다문화 결손 보완의 핵심 자산. |
| `topic` | 주제·영역 허브 | 분수, 수와 연산, 도형과 측정 | `30-topics/`에 위치. 진입점. |

(개인용 episodic/reflective/thesis는 기본 미사용. 교수학습 통찰을 적고 싶으면 `reflective`를
선택적으로 추가 가능 — 단 교육과정 사실 노드와 섞지 말 것.)
...
엣지는 본문 `## Edges`에 **Dataview 인라인 필드**(`관계:: [[대상]]`)로 기록(Juggl 렌더용).
...
subject: 수학                   # 국어 | 수학 | KSL | …
domain: 수와 연산               # 별책 영역명
gradeBand: "5-6"               # 1-2 | 3-4 | 5-6
...
규칙:
- **파일명 = id**. concept/skill/misconception은 **한글 개념명**(옵시디언 위키링크 자연스럽게),
  standard는 **성취기준 코드**(`6수01-04.md`), topic은 `30-topics/<주제>.md`.
  (개인 위키의 영문 kebab 규칙은 교육과정 한글 개념엔 가독성이 떨어져 **의도적으로 한글 파일명** 채택.)
- `summary`는 반드시 한 문장 — 다른 LLM이 본문 안 읽고 관련성 판단하는 핵심.
- `status`: 갓 추출 `seedling` → 검수 중 `growing` → NCIC 원문 대조·교사 검수 완료 `evergreen`.
...
### Ingest (별책 → 노드) — `/ingest`
1. 대상 별책 PDF를 `10-sources/`에서 확인(원천). 필요한 영역/학년군 페이지를 읽는다(Read로 PDF 추출).
...
- **status 승격은 원문 대조 후**: 추출=`seedling`, 검수=`growing`, NCIC/교사 검수=`evergreen`.
...
- 외부 호출(웹 fetch 등)은 하기 전에 사용자에게 알린다. 원천 별책은 로컬 PDF라 추가 비용 없음.
...
3회째 실패 시 `opus`로 승격하거나 오케스트레이터가 직접 수행. 단, **노드 간 일관성이 중요한
연속 분해는 위임보다 인라인이 안전**(콜드 스타트 주의).
